...
#if KCP_SENDING_VERBOSE
            var analyzeResult = Util.CutPacket(buffer);
            _ = Task.Run(() => 
            {
                Log.Verb($"KCP Sending pkt content: ");
                foreach (var part in analyzeResult)
                {
                    Log.Verb($"    {part.startindex} +{part.len}bytes: {part.startbyte}-{(byte)(part.startbyte + part.len)}");
                }
            });
#endif
