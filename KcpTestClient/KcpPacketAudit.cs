...
namespace KcpTestClient
{
    ...
    /// <summary>
    /// A blocking version of <see cref="KcpPacketAudit"/> and cause massive sending pressure.
    /// </summary>
    internal static class KcpPacketAuditBlocking
    {
        /// <summary>
        /// Push a KCP packet to audit. <paramref name="packet"/> will be deep cloned and can be disposed by its owner after invoke.
        /// </summary>
        ...
        public static void PushPacket(DateTime sentTime, Memory<byte> packet, int avalidLength)
        {
            try
            {
                Span<byte> pktContent = packet.Slice(0, avalidLength).Span;
                List<string> exceptions = new();
                #region 获取conv和token（以第一个kcp包为准）
                uint conv = BinaryPrimitives.ReadUInt32LittleEndian(pktContent);
#if MIHOMO_KCP
                ...
                sb.Append("\n-----------------------------------------\n");
                #endregion
                Log.Warn($"Detected <color=Red>invalid kcp packet</color> sent at {sentTime}, bytes_sent={pktContent.Length}", nameof(KcpPacketAudit));
                Log.Dbug(sb.ToString(), nameof(KcpPacketAudit));
            }
            catch (Exception ex)
            {
