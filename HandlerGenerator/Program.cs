    ...
    foreach (var grp in cmdData.cmdlist_order_new)
    {
        if (grp.Count() == 1)
        {
            ...
            if (!mergeChanges[curMethodNum].ContainsKey(tuple.messageName))
            {
                    ...
                    $"return handler_{tuple.messageName}.NewShiftToOld(body, body_offset, body_length);");
            }
            #endregion
        ...
        else
        {
            ...
            if (!merged_haveConflict)
            {
                ...
                foreach (var tuple in grp)
                {
                    writing_list.Add($"// return handler_{tuple.messageName}.NewShiftToOld(body, body_offset, body_length); ");
                }
    ...
    foreach (var grp in cmdData.cmdlist_order_new)
    {
        if (grp.Count() == 1)
        {
            ...
            if (!mergeChanges[curMethodNum].ContainsKey(tuple.messageName))
            {
                    ...
                    $"return handler_{tuple.messageName}.NewShiftToOld(body); ");
            }
            #endregion
            #region 2. No conflict, but ignore the case for human written before
            else
        ...
        else
        {
            ...
            if (!merged_haveConflict)
            {
                ...
                foreach (var tuple in grp)
                {
                    writing_list.Add($"// return handler_{tuple.messageName}.NewShiftToOld(body); ");
                }
                writing_list.Add("throw new NotSupportedException(\"The cmdid conflict " +
                    "haven't been solved. Search for 'request special handle' in " +
                    "ProtoshiftHandlers/ProtoDispatch/ShiftCmdId.cs to get info about " +
    ...
    foreach (var grp in cmdData.cmdlist_order_old)
    {
        if (grp.Count() == 1)
        {
            ...
            if (!mergeChanges[curMethodNum].ContainsKey(tuple.messageName))
            {
                    ...
                    $"return handler_{tuple.messageName}.OldShiftToNew(body, body_offset, body_length); ");
            }
            #endregion
            #region 2. No conflict, but ignore the case for human written before
            else
        ...
        else
        {
            ...
            if (!merged_haveConflict)
            {
                ...
                foreach (var tuple in grp)
                {
                    writing_list.Add($"// return handler_{tuple.messageName}.OldShiftToNew(body, body_offset, body_length); ");
                }
                writing_list.Add("throw new NotSupportedException(\"The cmdid conflict " +
                    "haven't been solved. Search for 'request special handle' in " +
                    "ProtoshiftHandlers/ProtoDispatch/ShiftCmdId.cs to get info about " +
    ...
    foreach (var grp in cmdData.cmdlist_order_old)
    {
        if (grp.Count() == 1)
        {
            ...
            if (!mergeChanges[curMethodNum].ContainsKey(tuple.messageName))
            {
                    ...
                    $"return handler_{tuple.messageName}.OldShiftToNew(body); ");
            }
            #endregion
            #region 2. No conflict, but ignore the case for human written before
            else
        ...
        else
        {
            ...
            if (!merged_haveConflict)
            {
                ...
                foreach (var tuple in grp)
                {
                    writing_list.Add($"// return handler_{tuple.messageName}.OldShiftToNew(body); ");
                }
                writing_list.Add("throw new NotSupportedException(\"The cmdid conflict " +
                    "haven't been solved. Search for 'request special handle' in " +
                    "ProtoshiftHandlers/ProtoDispatch/ShiftCmdId.cs to get info about " +
